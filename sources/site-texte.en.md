...
1. March to August 2026 CARIAD, Volkswagen Group lead developer paired with an apprentice, reviewed by a senior, through Lunatech At the software arm of the Volkswagen Group, migration of an access-management tool from a server-side rendered monolith to React and a REST API. The Strangler Fig pattern, endpoint by endpoint behind a feature flag, with instant rollback and no regression. Java · Spring Boot · React 19 · TypeScript · Vitest
2. July 2025 to March 2026 DARVA, Domaine Client backend developer, through Lunatech At an IT services provider for the insurance industry, migration of all customer data from the legacy CRM into the customer master data repository. Eight recurring batches brought under five minutes, and the migration batch for thirty thousand customers brought from eight hours down to under an hour and a half. A JVM heap saturation in UAT diagnosed on heap dumps and fixed in the data model. Java · Quarkus · Hibernate Reactive · Mutiny · MariaDB · Jenkins
3. January to July 2025 DARVA, Exiris full-stack developer, through Lunatech Electronic registered mail platform: search and certificate-monitoring features, and the move from Lighthouse to Playwright in the test suite. Java · Quarkus · React · TypeScript · Playwright · Jenkins
